...
include "../circuits/poseidon2/poseidon2_hash.circom";
...
template TestEnc() {
    ...
    // hash using poseidon
    var poseidon_inputs[3*n];
    for (var i = 0; i < n; i++) {
        poseidon_inputs[i] = should_be_c1[0][i];
        poseidon_inputs[n + i] = should_be_c1[1][i];
        poseidon_inputs[2*n + i] = should_be_c2[i];
    }

    var should_be_h = Poseidon2_hash(3*n)(poseidon_inputs);

    log("should_be_h: ", should_be_h, "\n");
    
    var RESULT = 1;
    RESULT = RESULT && (h == should_be_h);
