...
template TestEnc() {
    ...
    var sum = 0;
    for (var i = 0; i < 16; i++) {
        sum += should_be_h[i]*(1<<(8*i));
    }
    should_be_h0 = sum;

    sum = 0;
    for (var i = 16; i < 32; i++) {
        sum += should_be_h[i]*(1<<(8*(i-16)));
    }
    should_be_h1 = sum;

    var RESULT = 1;
    RESULT = RESULT && (h0 == should_be_h0);
    RESULT = RESULT && (h1 == should_be_h1);
